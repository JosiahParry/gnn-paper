...
as_sf <- function(d) {
  ...
  if (!is.na(CRS) && CRS == 4326) g <- st_transform(g, 3857)
  g
}
...
feature_cols <- feature_cols[vapply(st_drop_geometry(src_sf)[feature_cols], is.numeric, logical(1))]
...
make_graph <- function(geom, graph = GRAPH, kernel = "uniform", mult = 1) {
  ...
  if (kernel == "uniform") {
    ...
    kw <- sfdep::st_kernel_weights(nb, geom, kernel = kernel, threshold = thr, adaptive = FALSE)
    w <- unlist(kw)
    glist <- lapply(kw, function(v) pmax(v, 1e-8))   # spdep rejects all-zero rows
  }
  adj <- adj_from_edgelist(from, unlist(nb), weight = w) |> add_graph_self_loops()
  lw <- if (is.null(glist)) nb2listw(nb, zero.policy = TRUE)
        else nb2listw(nb, glist = glist, style = "W", zero.policy = TRUE)
  list(adj = adj, lw = lw)
}
...
fit_linear <- function(ids) {
  if (MODE == "clf") {
    m <- suppressWarnings(glm(y ~ ., data = frame_of(ids, "src"), family = binomial()))
    function(i, r) as.numeric(suppressWarnings(predict(m, frame_of(i, r), type = "response")))
  } else {
...
build_df <- function(ids, r, lw, lags) {
  ...
  out$y <- if (MODE == "clf") factor(y_of(r)[ids], levels = c(0, 1)) else y_of(r)[ids]
  out
}
...
xgb_spec <- function() {
  ...
  if (MODE == "clf") set_mode(s, "classification") else
    set_mode(s, "regression") |> set_engine("xgboost", objective = "reg:squarederror",
                                            validation = val_prop)
}
...
fit_xgb <- function(ids, lags, seed, k = default_k) {
  ...
  set.seed(seed)
  wf <- workflow() |>
    add_recipe(recipe(y ~ ., data = tr) |> step_normalize(all_numeric_predictors())) |>
    add_model(xgb_spec())
  fitted <- suppressWarnings(fit(wf, data = tr))
  function(i, r) {
    g2 <- sub_graph(geom_of(r)[i], k)
    nd <- build_df(i, r, g2$lw, lags)
    if (MODE == "clf") predict(fitted, nd, type = "prob")$.pred_1 else predict(fitted, nd)$.pred
  }
}

fit_sage <- function(train_id, val_id, norm, dropout = 0, wd = 0, k = default_k) {
  ...
  clf <- MODE == "clf"
  y_t <- if (clf) torch_tensor(y_of("src")[ids] + 1L, dtype = torch_long())
         else torch_tensor(y_of("src")[ids], dtype = torch_float32())$view(c(-1, 1))
  loss_fn <- if (clf) function(o, y) nnf_cross_entropy(o, y)
             else function(o, y) nnf_l1_loss(o, y)

  ti <- seq_along(train_id); vi <- seq_along(val_id) + length(train_id)
  model <- model_sage(in_features = length(feature_cols), hidden_dims = sage_hidden,
                      out_features = if (clf) 2L else 1L, norm = norm, dropout = dropout)
  opt_ <- optim_adam(model$parameters, lr = lr, weight_decay = wd)

  best <- Inf; state <- NULL; stall <- 0L
  for (e in seq_len(n_epochs)) {
    model$train(); opt_$zero_grad()
    o <- model(x_t, g$adj)
    l <- if (clf) loss_fn(o[ti, ], y_t[ti]) else loss_fn(o[ti, ], y_t[ti, ])
    l$backward(); opt_$step()
    with_no_grad({
      model$eval(); o2 <- model(x_t, g$adj)
      vl <- if (clf) loss_fn(o2[vi, ], y_t[vi])$item() else loss_fn(o2[vi, ], y_t[vi, ])$item()
    })
    if (vl < best) { best <- vl; state <- lapply(model$state_dict(), \(t) t$clone()); stall <- 0L }
    else stall <- stall + 1L
  ...
  function(i, r) {
    ...
    with_no_grad({
      o <- model(x2, g2$adj)
      if (clf) as.numeric(nnf_softmax(o, dim = 2)[, 2]) else as.numeric(o$squeeze())
    })
  }
}
...
arms <- list(
  ...
  `XGBoost + lags` = function(tr, va, seed, k) fit_xgb(c(tr, va), TRUE, seed, k),
  GraphSAGE = function(tr, va, seed, k) { torch_manual_seed(seed); fit_sage(tr, va, NULL, k = k) },
  `GraphSAGE + LayerNorm` = function(tr, va, seed, k) {
    torch_manual_seed(seed); fit_sage(tr, va, norm_node, 0.1, 1e-4, k)
  }
)
...
score <- function(truth, est) {
  if (MODE == "clf") {
    ...
    data.frame(accuracy = (tp + tn) / length(truth), bal_accuracy = mean(c(sens, spec)),
               auc = tryCatch(roc_auc_vec(factor(truth, levels = c(0, 1)), est,
                                          event_level = "second"), error = function(e) NA_real_),
               sensitivity = sens, specificity = spec, brier = mean((est - truth)^2))
  } else {
    m <- reg_metrics(data.frame(truth = truth, estimate = est), truth = truth, estimate = estimate)
    g <- function(n) m$.estimate[m$.metric == n]
    data.frame(mae = g("mae"), rmse = g("rmse"), rsq = g("rsq"), rsq_trad = g("rsq_trad"),
               bias = mean(est - truth), cal_slope = unname(coef(lm(truth ~ est))[2]))
  }
}
...
BANDWIDTHS <- list(list(kernel = "uniform", mult = 1),
...
k_label <- function(k) if (k$kernel == "uniform") "uniform" else sprintf("gauss_%g", k$mult)

set.seed(0)
.p <- sample(nrow(src_sf)); .n <- nrow(src_sf)
...
fit_tuned <- function(seed, arm) {
  ...
  sel <- function(p) {
    e <- p(tune_select, "src")
    if (MODE == "clf") mean((e > 0.5) == ys) else 1 - sum((ys - e)^2) / sum((ys - mean(ys))^2)
  }
  best <- NULL; bs <- -Inf
  for (k in BANDWIDTHS) {
    p <- if (grepl("GraphSAGE", arm)) {
      torch_manual_seed(seed); fit_sage(tune_train, tune_stop, norm_node, 0.1, 1e-4, k)
    } else fit_xgb(tune_train, TRUE, seed, k)
    s <- sel(p); if (is.finite(s) && s > bs) { bs <- s; best <- k }
  }
  p <- if (grepl("GraphSAGE", arm)) {
    torch_manual_seed(seed); fit_sage(c(tune_train, tune_stop), tune_select, norm_node, 0.1, 1e-4, best)
  } else fit_xgb(c(tune_train, tune_stop), TRUE, seed, best)
...
task <- function(spec) {
  ...
  if (isTRUE(spec$tuned)) {
    ...
    p <- r$predictor; chosen <- r$chosen
  } else {
    p <- arms[[spec$arm]](final_train, final_val, spec$seed, k); chosen <- k_label(k)
  }
  cbind(src = SRC, tgt = TGT, arm = spec$arm, seed = spec$seed,
        graph = GRAPH, chosen = chosen,
...
start_daemons <- function(n = max(1L, parallel::detectCores() - 2L)) {
  ...
  daemons(n)
  everywhere({
    options(gnn_data = a, gnn_src = b, gnn_tgt = cc, gnn_crs = d,
            gnn_graph = e, gnn_cell = f, gnn_mode = g)
    source(path, local = FALSE); torch_set_num_threads(1L)
  }, .args = list(path = core, a = DATA, b = SRC, cc = TGT, d = CRS,
                  e = GRAPH, f = CELL, g = MODE), .min = n)
  invisible(n)
}
