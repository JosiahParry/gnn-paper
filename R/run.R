...
options(
  ...
  gnn_graph = arg(5, "knn30"),
  gnn_cell = as.numeric(arg(6, "60")),
  gnn_mode = arg(7, "reg")
)
...
start_daemons()
jobs <- unlist(
  lapply(names(arms), function(x) {
    lapply(if (x %in% stochastic) seeds else seeds[1], function(s) {
      list(arm = x, seed = s)
    })
  }),
  recursive = FALSE
...
s <- res |>
  ...
  summarise(
    n = dplyr::n(),
    across(where(is.numeric) & !seed, mean),
    .groups = "drop"
  ) |>
  as.data.frame()
s <- s[order(-s[[metric]]), ]
