# Queens -> Brooklyn street-tree transfer (NYC 2015 Street Tree Census).
#
# Target: log trunk diameter (DBH), demeaned within borough. This is the
# project's first individual-record target that is NOT a price, which is what
# it is here to test: three of the four existing transfer wins are price
# targets, so the "individual-record targets favour a GNN" claim rested on a
# thin contrast.
#
# Passes the OLS compatibility screen in both directions (queens->brooklyn
# +0.275, brooklyn->queens +0.324), unlike LA->NYC Airbnb or the building
# energy pair.
#
# Same protocol as the other transfer cores: inductive KNN graph per region
...
cleaned <- readRDS("data/trees-clean.rds")
...
feature_cols <- setdiff(names(st_drop_geometry(qns)), c("boro", "y"))
...
start_daemons <- function(n_daemons = max(1L, parallel::detectCores() - 2L)) {
  core <- normalizePath("R/trees-core.R", mustWork = TRUE)
  daemons(n_daemons)
  everywhere(
