# LA -> NYC Airbnb listing-price transfer, same protocol as R/kc-to-ames.R
# and R/cdc-places.R. Every arm scored as a single deployable model; seeds
# characterize typical performance and spread, never combined.
#
# Run: Rscript R/airbnb.R [n_daemons]

source("R/airbnb-core.R")
...
cat(sprintf("\nLA -> NYC transfer: %d fits\n", length(jobs)))
t0 <- Sys.time()
res <- do.call(rbind, mirai_map(jobs, nyc_task)[.progress, .stop])
daemons(0)
...
cat("\n=== LA -> NYC, listing price: mean of independent single-model fits ===\n")
print(summary[order(-summary$rsq_trad), ], row.names = FALSE, digits = 3)

dir.create("data", showWarnings = FALSE)
saveRDS(list(folds = res, summary = summary), "data/airbnb-results.rds")
cat("\nSaved to data/airbnb-results.rds\n")
