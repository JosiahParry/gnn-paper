...
cross <- function(x, y, xn, yn) {
  ...
  data.frame(
    fit = xn,
    predict = yn,
    rsq_in = summary(m)$r.squared,
    rsq_out = r2(y$y, p),
    bias = mean(p - y$y),
    cal_slope = unname(coef(lm(y$y ~ p))[2])
  )
}
cat("=== will anything transfer ===\n")
print(
  rbind(cross(src, tgt, SRC, TGT), cross(tgt, src, TGT, SRC)),
  row.names = FALSE,
  digits = 3
)
...
cat(sprintf(
  "covariate Moran's I       median %+.3f  max %+.3f\n",
  median(cov_i, na.rm = TRUE),
  max(cov_i, na.rm = TRUE)
))
cat(sprintf(
  "covariates above residual %.2f\n",
  mean(cov_i > mi(res, lw), na.rm = TRUE)
))
cat(sprintf(
  "residual explained by neighbouring covariates  %.3f\n",
