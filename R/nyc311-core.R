# Queens -> Brooklyn 311 response-time transfer (NYC 311, DSNY, 2024).
#
# Target: log1p(hours from request to closure), demeaned within borough.
# The project's second non-price individual-record target, on the same region
# pair as the street-tree test, so the two differ only in what is predicted.
#
# Same protocol as every other transfer core: inductive KNN graph per region
# with no edges across the boundary, sfdep kernel weighting, node-mode
# LayerNorm + dropout 0.1 + weight decay 1e-4, single deployable models only
# (no prediction-level ensembling anywhere in this project).
#
# NOTE: 311 requests cluster on exact addresses, so duplicate coordinates are
# common. adaptive = TRUE is therefore not used -- a zero-width bandwidth
# produces NaN weights, the same failure seen on the Airbnb listings.
...
qns <- cleaned$queens   |> st_as_sf(coords = c("lon", "lat"), crs = 4326) |> st_transform(3857)
bkn <- cleaned$brooklyn |> st_as_sf(coords = c("lon", "lat"), crs = 4326) |> st_transform(3857)

feature_cols <- setdiff(names(st_drop_geometry(qns)), "y")

cat(sprintf("Queens (source)   n = %d\n", nrow(qns)))
cat(sprintf("Brooklyn (target) n = %d\n", nrow(bkn)))
cat(sprintf("features (%d): %s\n", length(feature_cols), paste(feature_cols, collapse = ", ")))

X_of <- function(region) as.matrix(st_drop_geometry(if (region == "qns") qns else bkn)[, feature_cols])
y_of <- function(region) if (region == "qns") qns$y else bkn$y
geom_of <- function(region) st_geometry(if (region == "qns") qns else bkn)
...
make_scaler <- function(ids) {
  mu <- colMeans(X_of("qns")[ids, , drop = FALSE])
  sdv <- apply(X_of("qns")[ids, , drop = FALSE], 2, sd)
  sdv[sdv == 0] <- 1
...
fit_ols <- function(fit_ids) {
  m <- lm(y ~ ., data = frame_of(fit_ids, "qns"))
...
fit_xgb <- function(fit_ids, with_lags, seed, kspec = default_kspec) {
  g <- make_sub_graph(geom_of("qns")[fit_ids], kspec$kernel, kspec$threshold_mult)
  train_df <- build_df(fit_ids, "qns", g$lw, with_lags)
  set.seed(seed)
...
fit_sage <- function(train_id, val_id, norm, dropout = 0, wd = 0, kspec = default_kspec) {
  ...
  g <- make_sub_graph(geom_of("qns")[ids], kspec$kernel, kspec$threshold_mult)

  x_t <- torch_tensor(scaler(ids, "qns"), dtype = torch_float32())
  y_t <- torch_tensor(y_of("qns")[ids], dtype = torch_float32())$view(c(-1, 1))
...
set.seed(0)
final_val <- sample(nrow(qns), size = floor(val_prop * nrow(qns)))
final_train <- setdiff(seq_len(nrow(qns)), final_val)

start_daemons <- function(n_daemons = max(1L, parallel::detectCores() - 2L)) {
  core <- normalizePath("R/nyc311-core.R", mustWork = TRUE)
  daemons(n_daemons)
  everywhere(
...
bkn_task <- function(spec) {
  ...
  cbind(arm = spec$arm, seed = spec$seed,
        ...
        score(y_of("bkn"), predictor(seq_len(nrow(bkn)), "bkn")))
}
