# Broward -> San Diego Airbnb listing-price transfer.
# Run: Rscript R/airbnb-brow-sd.R [n_daemons]
...
cat(sprintf("\nBroward -> San Diego transfer: %d fits\n", length(jobs)))
t0 <- Sys.time()
...
cat("\n=== Broward -> San Diego, listing price: mean of independent single-model fits ===\n")
print(summary[order(-summary$rsq_trad), ], row.names = FALSE, digits = 3)

dir.create("data", showWarnings = FALSE)
saveRDS(list(folds = res, summary = summary), "data/airbnb-brow-sd-results.rds")
cat("\nSaved to data/airbnb-brow-sd-results.rds\n")
