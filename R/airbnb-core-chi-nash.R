# Chicago -> Nashville Airbnb listing-price transfer. Same protocol as
# R/airbnb-core.R (LA -> NYC), smaller region pair (3k-10k rows, not 20k+ --
# Inside Airbnb's current US snapshot only has LA and NYC above 20k).
# Gaussian-kernel edge weights, node-mode LayerNorm + dropout=0.1 + wd=1e-4.
# No prediction ensembling -- single deployable models only.
...
cleaned <- readRDS("data/airbnb-chi-nash-clean.rds")
chi <- cleaned$chicago |> mutate(y = log(price) - mean(log(price))) |> st_as_sf(coords = c("lon", "lat"), crs = 4326) |> st_transform(3857)
nash <- cleaned$nashville |> mutate(y = log(price) - mean(log(price))) |> st_as_sf(coords = c("lon", "lat"), crs = 4326) |> st_transform(3857)

cat(sprintf("Chicago (source)   n = %d\n", nrow(chi)))
cat(sprintf("Nashville (target) n = %d\n", nrow(nash)))

X_of <- function(region) as.matrix(st_drop_geometry(if (region == "chi") chi else nash)[, feature_cols])
y_of <- function(region) if (region == "chi") chi$y else nash$y
geom_of <- function(region) st_geometry(if (region == "chi") chi else nash)
...
fit_xgb <- function(fit_ids, with_lags, seed) {
  g <- make_sub_graph(geom_of("chi")[fit_ids])
...
fit_sage <- function(train_id, val_id, norm, dropout = 0, wd = 0) {
  ...
  g <- make_sub_graph(geom_of("chi")[ids])
...
start_daemons <- function(n_daemons = max(1L, parallel::detectCores() - 2L)) {
  core <- normalizePath("R/airbnb-core-chi-nash.R", mustWork = TRUE)
  daemons(n_daemons)
  everywhere(
...
nash_task <- function(spec) {
  predictor <- arms[[spec$arm]](final_train, final_val, spec$seed)
  cbind(arm = spec$arm, seed = spec$seed, score(y_of("nash"), predictor(seq_len(nrow(nash)), "nash")))
}
