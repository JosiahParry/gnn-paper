...
MODE <- commandArgs(trailingOnly = TRUE)[1]
if (is.na(MODE)) {
  stop("pick one: airbnb nonprice cdc cdcclean nyc311 trees heat kcames")
}
...
if (MODE == "cdc") {
  ...
  measures <- c(
    ...
    "CSMOKING",
    "LPA",
    "SLEEP", # risk behaviors
    "ACCESS2",
    "CHECKUP", # access to care
    "FOODINSECU",
    "HOUSINSECU",
    "LACKTRPT", # social needs
...
if (MODE == "nyc311") {
  ...
  fetch_paged <- function(base, where, select, page = 50000) {
    ...
    repeat {
      url <- sprintf(
        "%s?$select=%s&$where=%s&$limit=%d&$offset=%d",
        base,
        select,
        where,
        page,
  ...
  whr <- sprintf(
    paste0(
      "agency='%s' AND created_date>='%s' AND created_date<'%s' ",
  ...
  qns <- split_boro("queens")
  bkn <- split_boro("brooklyn")
  cat(sprintf(
...
if (MODE == "trees") {
  ...
  prep <- function(d, label) {
    ...
    for (s in top_spc) {
      out[[paste0("spc_", gsub("[^a-z]", "_", tolower(s)))]] <- as.integer(
        !is.na(out$spc) & out$spc == s
      )
    }
    out <- out |> select(-spc)
...
if (MODE == "heat") {
  ...
  export_tif <- function(
    ...
    if (!is.null(interp)) {
      args <- c(args, q("interpolation=%s", interp))
    }
    if (!is.null(mosaic)) {
      args <- c(
        args,
        c("--data-urlencode", shQuote(paste0("mosaicRule=", mosaic)))
      )
    }
    ...
    if (st != 0 || !file.exists(dest) || file.size(dest) < 5000) {
      msg <- if (file.exists(dest)) {
        paste(readLines(dest, n = 8, warn = FALSE), collapse = " ")
      } else {
        "no file"
      }
  ...
  build <- function(nm, ctr) {
    ...
    lst <- if (file.exists(f_lst)) {
      ...
      export_tif(
        ...
        N,
        f_lst,
        rendering = "Band 10 Surface Temperature in Celsius",
        mosaic = MOSAIC_RULE
      )
    }
    names(lst) <- "lst"
    cat(sprintf(
      ...
      min(values(lst), na.rm = TRUE),
      max(values(lst), na.rm = TRUE)
    ))

    ms <- if (file.exists(f_ms)) {
      rast(f_ms)
    } else {
      export_tif(
        LS_URL,
    ...
    for (lo in unique(floor(c(llb$xmin - pad, llb$xmax + pad) / 5) * 5 + 2.5)) {
      for (la in unique(
        floor(c(llb$ymin - pad, llb$ymax + pad) / 5) * 5 + 2.5
      )) {
    ...
    dem <- project(
      crop(
        dem_ll,
        ext(llb$xmin - pad, llb$xmax + pad, llb$ymin - pad, llb$ymax + pad)
      ),
      crs(lst),
      res = 60,
      method = "bilinear"
    )
    elev <- resample(dem, lst, method = "average")
  ...
  if (max(sds) / min(sds) > 2) {
    warning(sprintf(
      paste0(
        "Temperature spread differs %.1fx between cities (%s). ",
        "Check the scenes are from the same season."
      ),
      max(sds) / min(sds),
      paste(sprintf("%.2f", sds), collapse = " vs ")
    ))
  }
  ...
  for (n in names(heat)) {
    cat(sprintf(
      "%-12s n = %5d | temp sd = %.2f C | built share = %.2f\n",
      n,
      nrow(heat[[n]]),
      sd(heat[[n]]$y),
      mean(lulc[[n]]$y)
...
if (MODE == "kcames") {
  ...
  kc <- readr::read_csv("data/kc_house_data.csv", show_col_types = FALSE) |>
    ...
    select(lon, lat, all_of(kc_feats), y) |>
    as.data.frame()
  ...
  cat(sprintf(
    "%-34s kc %d / ames %d, %d features\n",
    "data/rev-kc-ames.rds",
    nrow(kc),
    nrow(ames),
    length(kc_feats)
  ))
...
if (MODE == "cdcclean") {
  ...
  xy <- do.call(
    rbind,
    lapply(w$geolocation$coordinates, function(c) c(lon = c[1], lat = c[2]))
  )
  w <- w |>
    mutate(lon = xy[, "lon"], lat = xy[, "lat"]) |>
    select(-geolocation) |>
  ...
  cat(sprintf(
    "ca %d, wv %d, %d features -> data/cdc-clean.rds\n",
    nrow(out$ca),
    nrow(out$wv),
    length(feats)
  ))
